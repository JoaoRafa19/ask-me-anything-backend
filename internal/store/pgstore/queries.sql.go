...
const markMessageAsAwnsered = `-- name: MarkMessageAsAwnsered :exec
UPDATE
    messages
SET
    awnsered = true
...
func (q *Queries) MarkMessageAsAwnsered(ctx context.Context, id uuid.UUID) error {
	_, err := q.db.Exec(ctx, markMessageAsAwnsered, id)
	return err
}
